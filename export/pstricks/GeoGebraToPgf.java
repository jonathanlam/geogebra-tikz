...
public abstract class GeoGebraToPgf extends GeoGebraExport {
	...
	protected void drawIntegralFunctions(GeoNumeric geo) {
		...
		if (plotWithGnuplot) {
			...
			codeFilledObject.append(format(a));
			codeFilledObject.append(":");
			codeFilledObject.append(format(b));
			...
			codeFilledObject.append(value);
			codeFilledObject.append("})");
			//codeFilledObject.append("%%% draw integral functions part 1\n");
		}
		// function g
		...
		codeFilledObject.append(" -- cycle;\n");
		//codeFilledObject.append("%%% draw integral functions2 \n");
		endBeamer(codeFilledObject);
	}

	@Override
	protected void drawIntegral(GeoNumeric geo) {
		// draws command Integral(f, a, b)
	...
	protected void drawAngle(GeoAngle geo) {
		...
		if (drawAngleAs(geo, EuclidianStyleConstants.RIGHT_ANGLE_STYLE_SQUARE)) {
			...
			// draw in right angle
			// only draw in the two lines, instead of the full square
		...
		// draw arc for the angle
			...
			if (drawAngleAs(geo, EuclidianStyleConstants.RIGHT_ANGLE_STYLE_DOT)) {
				...
				code.append(" circle (");
				code.append(format(diameter / 2)); // round to 2dp String.format("%.3f", a)
				code.append(");\n");
				endBeamer(code);
			}
		}
	...
	protected void drawGeoConicPart(GeoConicPart geo) {
		...
		if (geo.getConicPartType() == GeoConicNDConstants.CONIC_PART_SECTOR) {
			...
			code.append(sb2);
			code.append("});\n");
			code.append("% conic builder\n");
		}
		endBeamer(code);
	}

	boolean isDomainFunction(String s) {
		if (!s.contains("If"))
			return false;
		s = s.replace('≤', '<');
		if (s.contains("<x<"))
    ...
    ArrayList<String> getDomainFunction (String s)  {
		ArrayList<String> l = new ArrayList<String>();
		s = s.replace('≤', '<');
		String[] split1 = s.split(",");
		String eqn = split1[1].substring(0, split1[1].length() - 1);
		String conditional = split1[0];
		conditional = conditional.replace("If(", "");

		// If((0.0)<x<(1.0)
		String[] condSplit = conditional.split("<");

		l.add(condSplit[0].replace("(", "").replace(")", ""));
		l.add(condSplit[2].replace("(", "").replace(")", ""));
		l.add(eqn);
		return l;
	}
	...
	private void drawFunction(GeoFunction geo, StringBuilder sb,
		...
		double xrangemax = 1.3, xrangemin = -1.3;


		//sb.append("%%% og toValueString\n%%%");
		String value = f.toValueString(getStringTemplate());
		//String value = f.toLaTeXString(true, getStringTemplate());
		//sb.append(value + "\n");
		value = killSpace(StringUtil.toLaTeXString(value, true));
		// e.g. x^(2.0)    or    If((0.0)≤x≤(1.0),x^(2.0))
		boolean isDomainFunctionDef = false;
		isDomainFunctionDef = isDomainFunction(value);

		if (isDomainFunctionDef) {
			ArrayList<String> domainStuff = getDomainFunction(value);
			value = domainStuff.get(2);
			xrangemax = Double.valueOf(domainStuff.get(1));
			xrangemin = Double.valueOf(domainStuff.get(0));
		}

		//sb.append(value + "\n%%% after removing if , end toValueString\n");
	...
	private void drawPgfStandard(GeoFunction geo, StringBuilder sb,
		...
		sb.append(format(xrangemin));
		sb.append(":");
		sb.append(format(xrangemax));
		sb.append("] plot");
		sb.append("(\\x,{");
	...
	boolean isPurple(GeoElementND geo) {
		int red = geo.getObjectColor().getRed();
		int green = geo.getObjectColor().getGreen();
		int blue = geo.getObjectColor().getBlue();
		return (red == 127 && green == 0 && blue == 255);
	}
		...
		if (transparency && geo.isFillable()) {
			...
			case STANDARD:
				if (info.getAlpha() > 0.0f) {
					...
					sb.append(format(info.getAlpha()));
				}
				break;
			case SYMBOLS:
			case CROSSHATCHED:
	...
	protected void colorCode(GColor c0, StringBuilder sb) {
		if (frame.isGrayscale()) {
			...
			if ((c0.equals(GColor.BLUE))
					|| (compareColor(red, green, blue, 77, 77, 255)) // free point
					|| (compareColor(red, green, blue, 125, 125, 255)) // semi-free-fixed point
			) {
				sb.append("blue");
				return;
			}

			if ((c0.equals(GColor.RED))
					|| (compareColor(red, green, blue, 204, 0, 0)) // free point
			) {
				sb.append("red");
				return;
			}
